clear all
xbpts=[-1000;1000;1000;-1000];
ybpts=[-1000;-1000;1000;1000];
poly1=[3;4;xbpts;ybpts];
d1=decsg(poly1);
ref=input('number of refinements?');
[p,e,t]=initmesh(d1);
if ref~=0
r=1;
while r<=ref
...
%end
%G=[Gu;Gv;Gh];
G=MM*PSI+(tau/2)*(AAx*K1*PSI+AAy*K2*PSI);

AAA=MM-(tau/2)*AAx*K1-(tau/2)*AAy*K2;
AAA2=AAA;
r=1;
s=1;
for i=1:length(on)
